...
export class UpgradeProfile
  ...
  get isHomeVerified(): boolean {
    ...
    const value: any = this.verificationStatus?.profile?.home_verified;

    return (
      value === 1 ||
      value === true ||
      String(value) === '1' ||
      this.verificationStatus?.verification_completed === true
    );
  ...
  get isPaymentUnderReview(): boolean {
    ...
    if (this.isHomeVerified) {
      return false;
    }

    return this.verificationStatus?.payment_status === 'paid';
  ...
  closePlanModalFromBackdrop(event: MouseEvent): void {
